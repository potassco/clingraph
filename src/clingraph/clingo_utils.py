...
class ClingraphContext:
    ...
    def replace(self, value, old, new):
        ...
        """
        return String(str(value).strip('"').replace(str(old).strip('"'), str(new).strip('"')))
    ...
    def format(self, s, *args):
        ...
        Returns:
            (clingo.Symbol.String) The string concatenating all symbols
        """
        if len(args) == 1 and args[0].type == SymbolType.Function and args[0].name == "":
    ...
    def svg(self, event, element, property_name, property_value):
        ...
        Args:
            ...
            element: The id on the element in which the action is performed.
                     This element must have the id property set: `attr(node,ID,id,ID):-node(ID).`
            property_name: The name of the css property to set.
            property_value: The value of the property to set
        Returns:
...
def _get_fbs_from_encoding(args, stdin, prgs_from_json):
    ...
    def add_fb_model(m):
        fbs.append(Factbase.from_model(m, prefix=args.prefix, default_graph=args.default_graph))
