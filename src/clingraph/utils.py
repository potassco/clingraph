...
def apply(elements, function, **kwargs):
    ...
    if "name_format" in kwargs:
        ...
        if len(elements) > 1 and "{model_number}" not in name_format:
            log.warning(
                "Output files will be overwritten since no `{model_number}` is used in the name format argument."
            )
    result = []
    for i, d in enumerate(elements):
        ...
        for graph_name, e in d.items():
            if name_format:
                kwargs["name_format"] = name_format.replace("{graph_name}", graph_name).replace(
                    "{model_number}", str(i)
                )
...
def write(elements, directory, format, name_format=None):
    ...
    """
    # pylint: disable=redefined-builtin
    return apply(elements, _write, directory=directory, format=format, name_format=name_format)
