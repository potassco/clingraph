...
def _get_parser():
    ...
    parser.add_argument(
        "-q", help="""Flag to have a quiet output where the graphs source wont be rendered""", action="store_true"
    )
    parser.add_argument(
        "-log",
        default="warning",
    ...
    graphviz_params = parser.add_argument_group(
        "OUTPUT {dot|render|tex|animate}", "Options for the functionality regarding graphviz."
    )

    graphviz_params.add_argument(
        "--type",
        default="graph",
    ...
    graphviz_render_params.add_argument(
        "--format",
        default="pdf",
        choices=["pdf", "png", "svg"],
        help=textwrap.dedent(
    ...
    graphviz_gif_params.add_argument(
        "--fps",
        default=1,
        help=textwrap.dedent(
            """\
    ...
    graphviz_tex_params.add_argument(
        "--tex-param",
        default="",
        help=textwrap.dedent(
            """\
...
def _get_fbs_normal(args, stdin, prgs_from_json):
    ...
    if prgs_from_json is not None:
        fbs = [
            Factbase.from_string(prg, prefix=args.prefix, default_graph=args.default_graph) for prg in prgs_from_json
        ]
    else:
...
def main():
    ...
    if args.select_model is not None:
        ...
        fbs = [f if i in args.select_model else None for i, f in enumerate(fbs)]
    ...
    if any("default" in graph and len(graph) > 1 for graph in graphs):
        log.warning(
            "Some nodes and edges were assigned to the default graph, since no specific graph was assigned as the last argument, "
    ...
    if args.out == "render":
        ...
        paths = render(graphs, format=args.format, engine=args.engine, view=args.view, **write_arguments)
        if not args.q:
            for p_dic in paths:
                if p_dic is None:
                    continue
    ...
    if args.out == "animate":
        ...
        paths = save_gif(graphs, engine=args.engine, fps=args.fps, sort=args.sort, **write_arguments)
        if not args.q:
            for p_dic in paths:
                if p_dic is None:
                    continue
