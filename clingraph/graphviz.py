        ...
        for e_type in elements:
            ...
            for e in element_ids:
                ...
                if e_type == "node":
                    ...
                    if (
                        e.symbol.type != SymbolType.Function
                        or len(e.symbol.arguments) < 2
                    ):
                    ...
                    graph.edge(
                        str(e.symbol.arguments[0]).strip('"'),
                        str(e.symbol.arguments[1]).strip('"'),
                        **attr,
                    )
...
def compute_graphs(fb, graphviz_type="graph", seed=None):
    ...
    for f in fb:
        if f is None:
            ...
            result.append(
                _compute_graphs_single_fb(f, graphviz_type=graphviz_type, seed=seed)
            )
    if not is_multi_model:
        return result[0]
    return result
...
def _render_single_graph(graph, directory, format, engine, view, name_format=None):
    ...
    file_path = os.path.join(directory, f"{file_name}.{format}")
    graph.engine = engine
    graph.render(
        format=format,
        directory=directory,
...
def render(
    graphs, directory="out", format="pdf", name_format=None, engine="dot", view=False
):
    """
